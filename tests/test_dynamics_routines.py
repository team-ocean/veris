...
def test_ice_strength_concentration_and_land(state, sett):
    ...
    result = SeaIceStrength(state(hIceMean=ice, Area=area, iceMask=mask), sett)
    expected = np.zeros_like(ice)
    for i, j in np.ndindex(ice.shape):
        if mask[i, j]:
            expected[i, j] = (
                sett.pStar * ice[i, j] * np.exp(-sett.cStar * (1 - area[i, j]))
            )
...
def test_ocean_drag_relative_speed_floor_and_land(state, sett, coriolis, velocity):
    ...
    result = ocean_drag_coeffs(
        vs, sett, (0.2 + velocity[0]) * ones, (-0.1 + velocity[1]) * ones
    )
    coefficient = sett.waterIceDrag_south if coriolis < 0 else sett.waterIceDrag
    expected = max(sett.cDragMin, sett.rhoSea * coefficient * np.hypot(*velocity))
...
def test_basal_drag_regularized_keel_threshold(state, sett, area, velocity):
    ...
    result = basal_drag_coeffs(vs, sett, velocity[0] * ones, velocity[1] * ones)
    expected = 0
    if area > 0.01:
        speed = np.sqrt(0.5 * np.hypot(*velocity) ** 2 + sett.basalDragU0**2)
        keel_excess = 2 - 16 * area / sett.basalDragK1
        expected = (
            sett.basalDragK2
            / speed
            * np.logaddexp(0, 10 * keel_excess)
            / 10
            * np.exp(-sett.cBasalStar * (1 - area))
        )
    assert result.shape == ones.shape
...
def test_side_drag_coastline_and_neighbor_counts(state, sett, coastline, velocity):
    ...
    result = side_drag(vs, sett, velocity[0] * ones, velocity[1] * ones)
    ...
    for i, j in np.ndindex(ones.shape):
        ...
        denominator = np.hypot(*velocity) + sett.sideDragU0
        expected_u[i, j] = 900 * sett.sideDragCoeff * factor_u / denominator
        expected_v[i, j] = 600 * sett.sideDragCoeff * factor_v / denominator
...
def test_affine_cartesian_strain_tensor(state, sett, boundary, coefficients):
    ...
    result = strainrates(vs, sett, a * x + b * y + 0.2, c * x + d * y - 0.3)
...
def test_uniform_viscosity_and_stress_scalar_equations(
    ...
    e11, e22, e12 = strain
    delta = np.sqrt(
        (e11 + e22) ** 2 + ((e11 - e22) ** 2 + 4 * e12**2) / sett.PlasDefCoeff**2
    )
    bulk = strength * (1 + tensile) / (2 * (delta + sett.deltaMin))
    shear = bulk / sett.PlasDefCoeff**2
    pressure = (
        strength
        * (1 - tensile)
        * (1 - replacement + replacement * delta / (delta + sett.deltaMin))
    )
    result = viscosities(vs, sett, e11 * ones, e22 * ones, e12 * ones)
    for value, reference in zip(result, (bulk, shear, pressure)):
        assert value.shape == ones.shape
        np.testing.assert_allclose(value, reference, rtol=1e-13)
    tensor = stress(vs, sett, e11 * ones, e22 * ones, e12 * ones, *result)
    expected = (
...
def test_stress_divergence_affine_cartesian_tensor(state, sett, constant):
    ...
    if constant:
        ...
        expected = (6, -2.5)
    for value, reference in zip(stressdiv(vs, sett, *tensor), expected):
        assert value.shape == x.shape
