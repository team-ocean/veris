...
def launch_processes(platform="cpu", count=2, devices_per_process=1, timeout=90):
...
def check_layout(px, py):
    ...
    data = jax.make_array_from_callback(shape, sharding, lambda index: packed[index])

    def local_norms(value):
    ...
    norms = jax.shard_map(local_norms, mesh=mesh, in_specs=P("x", "y"), out_specs=P())
    n = px * py * nx * ny
    expected = [n * (n + 1) / 2, n * (n + 1) * (2 * n + 1) / 6]
    np.testing.assert_allclose(norms(data), expected, rtol=1e-13)
    tangent = jax.make_array_from_callback(
        shape, sharding, lambda index: np.ones(shape)[index]
    )
    with jax.set_mesh(mesh):
        _, directional = jax.jvp(norms, (data,), (tangent,))
        adjoint = jax.grad(lambda value: norms(value)[1])(data)
