...
def equilibrium_state(
    state: StateFactory, conf: Configuration, phys: PhysicalConstants, **changes: float
) -> Any:
...
def test_zero_forcing_preserves_equilibrium(
    ...
    conf = replace(conf, nITC=categories)
    vs = equilibrium_state(state, conf, phys, Area=area, hIceMean=area)
    result = Growth(vs, conf, phys)
    expected = (
        area,
        0,
        area,
...
def test_open_water_freezing_conserves_latent_heat_and_freshwater(
    ...
    conf = replace(conf, deltatTherm=600)
    ...
    frozen_mass = cooling * conf.deltatTherm / phys.lhFusion
...
def test_ice_free_heating_and_shortwave_pass_to_ocean(
    state: StateFactory, conf: Configuration, phys: PhysicalConstants, heating: float
) -> None:
    vs = equilibrium_state(
        state, conf, phys, hIceMean=0, Area=0, Qnet=-heating, Qsw=-80
    )
    result = Growth(vs, conf, phys)
...
def test_cold_snowfall_stores_water_on_ice_and_rain_reaches_leads(
    ...
    snowfall = 1e-7
    vs = equilibrium_state(
        state, conf, phys, Area=area, snowfall=snowfall, precip=precip
    )
    result = Growth(vs, conf, phys)
    stored_mass = (snowfall + precip) * area * conf.deltatTherm * phys.rhoFresh
...
def test_flooding_converts_submerged_snow_and_preserves_column_mass(
    ...
    ice = 0.5
    vs = equilibrium_state(state, conf, phys, hIceMean=ice, hSnowMean=snow)
    result = Growth(vs, conf, phys)
...
def test_mixed_layer_melt_uses_ocean_heat_and_returns_freshwater(
    state: StateFactory, conf: Configuration, phys: PhysicalConstants, warming: float
) -> None:
    conf = replace(conf, deltatTherm=600)
    vs = equilibrium_state(
        state, conf, phys, theta=phys.celsius2K + phys.tempFrz + warming
    )
    result = Growth(vs, conf, phys)
    ...
    melted_mass = heat * conf.deltatTherm / phys.lhFusion
...
def test_partial_cover_conduction_closes_latent_heat_budget(
    ...
    atmospheric_loss = area * conduction * conf.deltatTherm
...
def test_partial_cover_surface_heat_melts_snow_with_one_area_weight(
    ...
    conf = replace(conf, nITC=1, deltatTherm=600)
    ...
    result = Growth(vs, conf, phys)
    melted_mass = heating * area * conf.deltatTherm / phys.lhFusion
    np.testing.assert_allclose(result[0], 1, atol=1e-12)
    np.testing.assert_allclose(
        (vs.hSnowMean - result[1]) * phys.rhoSnow, melted_mass, rtol=1e-11
    )
    np.testing.assert_allclose(result[4], -melted_mass / conf.deltatTherm, atol=1e-12)
...
def test_complete_snow_melt_and_excess_ice_melt_close_energy(
    ...
    heat = heat_multiple * snow_latent / (area * conf.deltatTherm)
    vs = equilibrium_state(
        state,
        conf,
        phys,
    ...
    result = Growth(vs, conf, phys)
    total_energy = heat * area * conf.deltatTherm
