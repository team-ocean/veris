...
def test_remove_halos_and_save_selected_fields(tmp_path: Path) -> None:
    ...
    destination = tmp_path / "subdir" / "output.npz"
    ...
    with np.load(destination) as result:
        assert set(result.files) == set(run_parallel.OUTPUT_FIELDS)
        np.testing.assert_array_equal(result["hIceMean"], expected)
...
def test_invalid_mesh_and_cli_dimensions() -> None:
    ...
    assert arguments.output == Path("parallel.npz")
...
def test_gpu_cli_writes_finite_output(tmp_path: Path) -> None:
    ...
    with np.load(destination) as fields:
        assert set(fields.files) == set(run_parallel.OUTPUT_FIELDS)
        assert all(fields[name].shape == (12, 16) for name in fields.files)
        assert all(np.isfinite(fields[name]).all() for name in fields.files)
