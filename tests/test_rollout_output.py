"""Host scan chunks preserve selected output fields, schedules and warmup."""
...
def test_chunks_tail_and_warmup_start_from_original(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Warmup covers full and tail shapes without entering the actual trajectory."""
    ...
    def traced_step(initial: Any, advance: Any, steps: int, **kwargs: Any) -> Any:
        calls.append((int(initial["Area"][0, 0]), steps, kwargs["observe"]))
    ...
    assert [(value, length) for value, length, _ in calls] == [
        (0, 2),
        (0, 1),
        (0, 2),
        (2, 2),
