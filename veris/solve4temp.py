...
def solve4temp(vs, sett, hIceActual, hSnowActual, TSurfIn, TempFrz):
    ...
    ATempLoc = jnp.maximum(sett.celsius2K + sett.minTAir, vs.ATemp)
    ...
    d3 = jnp.where(isSnow, sett.snowEmiss, sett.iceEmiss) * sett.stefBoltz

    LWdownLoc = jnp.where(isSnow, sett.snowEmiss, sett.iceEmiss) * LWdownLocCapped

    ##### determine albedo #####

    # use albedo of dry surface (if ice is present)
    albIce = jnp.where(isIce, sett.dryIceAlb, 0)
    albSnow = jnp.where(isIce, sett.drySnowAlb, 0)

    # use albedo of wet surface if surface is thawing
    useWetAlb = (hIceActual > 0) & (TSurfLoc >= SurfMeltTemp)
    albIce = jnp.where(useWetAlb, sett.wetIceAlb, albIce)
    albSnow = jnp.where(useWetAlb, sett.wetSnowAlb, albSnow)

    # same for southern hermisphere
    south = (hIceActual > 0) & (vs.fCori < 0)
    albIce = jnp.where(south, sett.dryIceAlb_south, albIce)
    albSnow = jnp.where(south, sett.drySnowAlb_south, albSnow)
    useWetAlb_south = (hIceActual > 0) & (vs.fCori < 0) & (TSurfLoc >= SurfMeltTemp)
    albIce = jnp.where(useWetAlb_south, sett.wetIceAlb_south, albIce)
    albSnow = jnp.where(useWetAlb_south, sett.wetSnowAlb_south, albSnow)

    # if the snow thickness is smaller than hCut, use linear transition
    # between ice and snow albedo
    alb = jnp.where(isIce, albIce + hSnowActual / sett.hCut * (albSnow - albIce), 0)

    # if the snow thickness is larger than hCut, the snow is opaque for
    # shortwave radiation -> use snow albedo
    alb = jnp.where(hSnowActual > sett.hCut, albSnow, alb)
    ...
    penetSWFrac = jnp.where(isIce, sett.shortwave * jnp.exp(-1.5 * hIceActual), 0)
    ...
    effConduct = jnp.where(
        ...
        sett.iceConduct
        * sett.snowConduct
        / (sett.snowConduct * hIceActual + sett.iceConduct * hSnowActual),
        0,
    )
    ...
    for _ in range(6):
        ...
        TSurfLoc = jnp.maximum(TSurfLoc, sett.celsius2K + sett.minTIce)
    ...
    FWsublim = jnp.where(isIce, F_lh / sett.lhSublim, 0)
